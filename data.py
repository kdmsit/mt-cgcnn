...
def get_train_val_test_loader(dataset, collate_fn=default_collate,
								...
								val_size=1000, test_size=1000, return_test=False, return_val=False,
								num_workers=1, pin_memory=False):
	"""
	Utility function for dividing a dataset to train, val, test datasets.
	Also, for special case of sparse dataset, only generate a train loader.
	...
	return_test: bool
		...
		data will be hidden.
	return_val: bool
		Whether to return the val dataset loader.
	num_workers: int
	pin_memory: bool
	...
	# If no val or test loader, then all data used for training
	if not return_val and not return_test:
...
def collate_pool(dataset_list):
	...
	dataset_list: list of tuples for each data point.
		(atom_fea, nbr_fea, nbr_fea_idx, targets)
	...
	-------
	N = sum(n_i); N0 = sum(i)
	N-> total number of atoms across the batch (not necessarily unique atoms)
	N0->list size
	...
	targets: torch.Tensor shape (N, n_p)
